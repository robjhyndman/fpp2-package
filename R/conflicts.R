...
#' existing code.
#'
#' @return A list object of class \code{fpp2_conflicts}.
#' @export
#' @examples
#' fpp2_conflicts()
fpp2_conflicts <- function() {
  ...
  tidy_names <- paste0("package:", fpp2_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))

  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)

  structure(conflict_funs, class = "fpp2_conflicts")
}

fpp2_conflict_message <- function(x) {
  if (length(x) == 0) {
    return("")
  }
...
#' @export
print.fpp2_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(fpp2_conflict_message(x))
}
